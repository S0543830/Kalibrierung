...
#define RSL430_BUFFER_SIZE 2048000 // Als Konstante definieren
...
int CRSL430Scan::initRSL430UDP( const char* szHostName,  unsigned int uiPort )
{
	...
	if( INVALID_SOCKET == this->RSL430Socket )
	{
		...
		this->RSL430Addr.sin_port = static_cast<u_short>(htons( this->uiPortNr )); //Warning unsigned int to u_short
		this->RSL430Addr.sin_addr.s_addr = htonl(INADDR_ANY);

		sockaddr_in add;
        add.sin_family = AF_INET;
        add.sin_addr.s_addr = htonl(INADDR_ANY);
        add.sin_port = static_cast<u_short>(htons(this->uiPortNr));//Warning unsigned int to u_short
...
int CRSL430Scan::exitRSL430( void )
{
	...
	closesocket( this->RSL430Socket ); //Rückgabewert abgreifen und weitergeben
	this->RSL430Socket = INVALID_SOCKET;
...
/***********************************************************/
int CRSL430Scan::getSingleScan( std::list<SPunkt>& lcpPoints, std::list<double>& ldTimeStamps )
{
	...
	int senderAddrSize /*iSenderAddrSize*/ = sizeof(senderAddr);
...
/***********************************************************/
int CRSL430Scan::getRecordedScanPoints( std::list<SPunkt>& lcpRecPoints, std::list<double>& ldRecTimeStamps )
{
	int iReturn = RSL430_RETURN_OK;
...
int CRSL430Scan::loadCorrectValueDat(const char* szDatName)
{
	...
	fDat.open(szDatName, std::ios::in);
