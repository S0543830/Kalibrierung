...
typedef int(*iGETFILTERREC)(unsigned int, std::list<SPunkt>&, std::list<double>&, unsigned int);
...
int CScanner::getFilterRecordedPoints(std::list<SPunkt>& lPunkte, std::list<double>& ldZeiten, unsigned int uiAnzahl)
{
	int iReturn = -1;
	lPunkte.clear();
	ldZeiten.clear();
	...
	if (NULL != this->m_pRotoPluginDll)
	{
		...
		if (NULL != pGetRecPoint)
		{
			iReturn = pGetRecPoint(this->m_iHandle, lPunkte, ldZeiten,uiAnzahl);
		}
	}
	return iReturn;
}
