...
class CSickScan
{
	public:
		...
		int getRecordedScanPoints( std::list <SPunkt>& rlstclPoints,
								   std::list <double>& rlstdTimeStamps,
								   bool bInterpoleateTimeStamps );
		int getFilterRecordPoints(std::list <SPunkt>& rlstclPoints,std::list <double>& rlstdTimeStamps,int  iAnzahl);
		int getStatus( void );
		unsigned int readMistake();
		unsigned int readBigMistake();
		int getSingleScan( std::list<SPunkt> &lcpPoints, 
	...
	private:
		...
		int convertDataFromLMS400(SickScanDataLMS400 lms, std::list<SPunkt> &lScanOut, std::list<double> &ldTimeStamps, int iMittelwertAnzahl);
