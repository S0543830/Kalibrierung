...
int CSickScan::initSickScan(const char *szHostName, const char *szPort)
{
	...
	if (0 == iReturn)
	{
		this->configSick(0.1f, 2, 55.0f, 70.0f);
...
/***********************************************************/
int CSickScan::convertDataFromLMS400(SickScanDataLMS400 lms,std::list<CPunkt> &lScanOut,std::list<double> &ldTimeStamps)
{
	CPunkt cpTmp;
		...
		if (this->iMinIntensity < lms.vecIntensities.at(i))
		{
			if ((dAktAngle > this->dScanMinAngle) && (dAktAngle < this->dScanMaxAngle))
			{
				if ((usTmpDist >= this->dScanMinDist) && (usTmpDist <= this->dScanMaxDist))
				{
					...
					ldTimeStamps.push_back(lms.dStamp);
				}
			}
		}
...
/***********************************************************/
int CSickScan::getSingleScan( std::list<CPunkt> &lcpPoints, 
							...
							float fAngleLen)
{
	...
	if (SICK_OK == iVar)
	{
		...
		if (-1 == receiveFromSickLMS(this->SickSocket))
		{
			iReturn = ERR_ROTOSCAN_TELEGRAM_CRC_ERROR;
		}
		//Abfragen der Telegram Antwort
		else if (-1 == receiveFromSickLMS(this->SickSocket))
		{
...
/*Konvertiert diese in CPunkt                              */
/***********************************************************/
int CSickScan::getRecordedScanPoints( std::list <CPunkt>& rlstclPoints, 
